...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (q_input, target) in enumerate(train_loader):
        ...
        top5.update(acc5[0], q_input.size(0))
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (q_input, target) in enumerate(val_loader):
...
def save_checkpoint(state, filename='checkpoint.pth.tar'):
    torch.save(state, './checkpoints/recognition/'+filename)
